...
const Aseo = () => {
    ...
    const[newLista, setnewLista] = useState(product.filter(product => product.clasificacion === "aseo"))
    ...
    useEffect(() =>{
        ...
        .then(res => {
            setProduct(res.data)
            setItems(res.data.filter(product => product.clasificacion === "aseo").slice(0, pages));
        })
        .catch(err => 
            {
